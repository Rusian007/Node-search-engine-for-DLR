...
class SearchController < ApplicationController
    ...
    def postSearch
        ...
        url = "http://localhost:8983/solr/dlr_search/select?indent=true&q.op=OR&q=section:#{queryField}%2C%0Atitle:#{queryField}"
        ...
        if !body['response']['docs'].empty?
            for record in body['response']['docs'] do 
                ...
                objArray << SearchResults.new(record['title'][0], record['section'], subsec)
